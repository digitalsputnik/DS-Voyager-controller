...
public static class AndroidVideoResizer
{
    ...
    public static void PickVideo(MonoBehaviour source, Action<bool, string> _callback)
    {
        if (compressedVideo == null)
        {
            _compressionFinishedCallback = _callback;

            source.StartCoroutine(SetupAndPickVideo());
        }
        else
    ...
    static IEnumerator SetupAndPickVideo()
    {
        ...
        PluginInstance.CallStatic("pickVideo", path);
    }

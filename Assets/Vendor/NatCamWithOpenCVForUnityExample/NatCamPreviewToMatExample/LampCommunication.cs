...
public class LampCommunication: MonoBehaviour {
    ...
    public Dictionary<string, int> LampMactoLengthDictionary = new Dictionary<string, int>();
    Dictionary<string, IPAddress> LampMactoIPDictionary = new Dictionary<string, IPAddress>(); //?
    ...
    Dictionary<string, ExtraProperties> MacToProps = new Dictionary<string, ExtraProperties>();
    ...
    void Start ()
    {
        ...
        //var tempLampDict = GameObject.Find("DetectedLampProperties").GetComponent<DetectedLampProperties>().LampMactoLengthDictionary;
        LampMactoLengthDictionary = GameObject.Find("DetectedLampProperties").GetComponent<DetectedLampProperties>().LampMactoLengthDictionary; // new Dictionary<string, int>(tempLampDict);
        LampMactoIPDictionary = GameObject.Find("DetectedLampProperties").GetComponent<DetectedLampProperties>().LampMactoIPDictionary;
        NewLampMactoLengthDictionary = new Dictionary<string, int>(LampMactoLengthDictionary);  //new Dictionary<string, int>(tempLampDict);
    ...
    void Update()
    {
        ...
        SendPermutationsToNewLamps(new Dictionary<string, int>(NewLampMactoLengthDictionary));
    }

    public void SendPermutationsToNewLamps(Dictionary<string,int> lampMactoLengthDictionary)
    {
        if (lampMactoLengthDictionary.Count > 0)
        {
            foreach (var newLamp in lampMactoLengthDictionary)
            {
                if (permutationCounter >= ColorPermutations.Count())
                    break;
                ...
                if (NewLampMactoLengthDictionary == null)
                {
                    for (int i = 0; i < 5; i++)
                    {
                        SendUDPMessage(cDTO);
                ...
                if (MacToProps.ContainsKey(newLamp.Key))
                {
                    lampColors.Add(new LampColorProperties
                    {
                        ...
                        ColorPattern = colorPattern,
                        BatteryLevel = MacToProps[newLamp.Key].BatteryLevel,
                        MacName = MacToProps[newLamp.Key].LampMac
                    });
                }
                ...
                if (NewLampMactoLengthDictionary != null)
                {
                    if (NewLampMactoLengthDictionary.Keys.Contains(newLamp.Key))
                    {
                        NewLampMactoLengthDictionary.Remove(newLamp.Key);
                    }
                }
    ...
    private UDPpacketDTO GenerateColorPacket(string lampMac, int lampLength, byte[][] colorPattern, int colorCount)
    {
        ...
        var remoteEndPoint = new IPEndPoint(LampMactoIPDictionary[lampMac], 30000);
    ...
    IEnumerator PollLamps(UDPpacketDTO udpDTO)
    {
        while (true)
        {
            ...
            while (udpDTO.client.Available > 0)
            {
                ...
                {
                    MacToProps.Add(lampMacName, new ExtraProperties
                    {
                        IP = LightIP,
                ...
                if (!LampMactoLengthDictionary.Keys.Contains(lampMacName))
                {
                    LampMactoLengthDictionary.Add(lampMacName, numPixels);
                    NewLampMactoLengthDictionary.Add(lampMacName, numPixels);
                    //GameObject.Find("DetectedLampProperties").GetComponent<DetectedLampProperties>().LampMactoLengthDictionary.Add(lampMacName, numPixels);

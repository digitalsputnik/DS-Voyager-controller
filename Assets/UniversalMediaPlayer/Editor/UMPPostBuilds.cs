...
public class UMPPostBuilds : MonoBehaviour
{
    ...
    public static void BuildWindowsPlayer32(string path)
    {
        ...
        if (!string.IsNullOrEmpty(buildPath))
        {
            ...
            {
                CopyPlugins(UMPSettings.Instance.AssetPath + "/Plugins/Win/x86/plugins/", dataPath + "/Plugins/plugins/");
            }
            else
            {
    ...
    public static void BuildWindowsPlayer64(string path)
    {
        ...
        if (!string.IsNullOrEmpty(buildPath))
        {
            if (!UMPPreference.Instance.UseExternalLibs)
            {
                CopyPlugins(UMPSettings.Instance.AssetPath + "/Plugins/Win/x86_64/plugins/", dataPath + "/Plugins/plugins/");
            }
            else
            {
                if (File.Exists(dataPath + "/Plugins/libvlc.dll"))
    ...
    public static void BuildLinuxPlayer32(string path)
    {
        string buildPath = Path.GetDirectoryName(path);
        string dataPath = buildPath + "/" + Path.GetFileNameWithoutExtension(path) + "_Data";
        string umpLauncherPath = UMPSettings.Instance.AssetPath + "/Plugins/Linux/UMPInstaller.sh";
        string umpRemoverPath = UMPSettings.Instance.AssetPath + "/Plugins/Linux/UMPRemover.sh";
        ...
        if (!string.IsNullOrEmpty(buildPath))
        {
            if (!UMPPreference.Instance.UseExternalLibs)
            {
                ...
                CopyPlugins(UMPSettings.Instance.AssetPath + "/Plugins/Linux/x86/plugins/", vlcFolderPath32 + "/plugins/");
            }
            else
            {
    ...
    public static void BuildLinuxPlayer64(string path)
    {
        ...
        if (!string.IsNullOrEmpty(buildPath))
        {
            if (!UMPPreference.Instance.UseExternalLibs)
            {
                ...
                CopyPlugins(UMPSettings.Instance.AssetPath + "/Plugins/Linux/x86_64/plugins/", vlcFolderPath64 + "/plugins/");
            }
            else
            {

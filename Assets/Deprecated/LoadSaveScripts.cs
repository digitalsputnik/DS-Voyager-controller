...
public class LoadSaveScripts : MonoBehaviour {
    ...
    public LampData GetLampData(GameObject lampObject)
    {
        ...
        lampData.IP = setupScripts.LampMactoIPDictionary[lampObject.GetComponent<Ribbon>().Mac].ToString(); //lampObject.GetComponent<Ribbon>().IP;
    ...
    public void LoadLampsToWorkspace(WorkspaceData wsData)
    {
        ...
        foreach (LampData lamp in wsData.Lamps)
        {
			...
			if (!setupScripts.LampMactoLengthDictionary.ContainsKey(lamp.Mac))
            {
                setupScripts.LampMactoLengthDictionary.Add(lamp.Mac, lamp.LampLength);
                setupScripts.LampMactoIPDictionary.Add(lamp.Mac, IPAddress.Parse(lamp.IP));
            }
            ...
            if (!ColorDataReceiver.GetComponent<GetLampColorData>().Mac.Contains(lamp.Mac))
            {
                ColorDataReceiver.GetComponent<GetLampColorData>().Mac.Add(lamp.Mac);
            }
            ////Set pixel colors

...
{
    private const string JSON = "{\"itshe\":{\"e\":1.0,\"h\":0.0,\"i\":0.5,\"s\":1.0,\"t\":0.5},\"op_code\":\"set_itshe\",\"timestamp\":100}";
    ...
    private void Start()
    {
        _stream = new SerialPort(_serialPort, 750000, Parity.None, 8, StopBits.One)
        {
    ...
    private void Update()
    {
        if (_stream.IsOpen)
        {
            ...
            {
                var packet = AssembleVoyagerPacket(JSON);
                _stream.Write(packet, 0, packet.Length);
            }

            try
            {
                var result = _stream.ReadLine();
    ...
    private byte[] AssembleVoyagerPacket(string text)
    {
        //header and footer
